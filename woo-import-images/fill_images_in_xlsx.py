#!/usr/bin/env python3
"""
Fill the 'Image' column in an Excel sheet by finding a first image URL
for each product Title (using a simple Bing Images page scrape).

Usage (inside your virtual environment):
    python3 fill_images_in_xlsx.py
...
# ================== CONFIG ==================
INPUT_XLSX = "products_for_auto_images.xlsx"  # Adjust if you rename the file
OUTPUT_XLSX = "products_with_images.xlsx"
TITLE_HEADER = "Title"
IMAGE_HEADER = "Image"
REQUEST_TIMEOUT = 20
...
def main():
    ...
    print(f"Found {rows-1} data rows. Filling '{IMAGE_HEADER}' from '{TITLE_HEADER}'...")
    ...
    for row in range(2, rows + 1):
        ...
        print(f"- [{row}] Searching image for: {title_str!r}")
    ...
    wb.save(OUTPUT_XLSX)
    print(f"Done. Filled {filled} rows, skipped {skipped}. Saved: {OUTPUT_XLSX}")
